...
_ADD3_CRITERIA: dict[int, str] = {
    ...
    3: "Two-tier rule. (A) Triggers WITH associated performance metrics (latency, TPM, concurrent users, timeouts): must map the triggering event to a specific component from campo_1, indicate whether the interaction is sync or async, and reference the endpoint or event name. (B) Triggers WITHOUT metrics (timer/schedule events, webhooks, deployments, CI/CD pipelines, malicious actors, administrative actions): sufficient if the stimulus is named specifically and is recognizable as a concrete system event — no component mapping required. In both cases, generic phrases like 'when the user sends a request' with no system-specific detail are NOT sufficient.",
