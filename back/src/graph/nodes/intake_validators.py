...
_ADD3_CRITERIA: dict[int, str] = {
    ...
    3: "Two-tier rule — diagnosis level only, not solution design. (A) Triggers WITH performance metrics (latency, TPM, concurrent users, timeouts): identify what functional area or part of the system receives or is affected by the event (e.g. 'the search functionality', 'the reservation flow', 'the payment processing') — do NOT require named architectural components like API Gateway, microservices, or specific services, as those emerge from ADD design. Indicate sync or async interaction, reference the endpoint or event name (approximate is acceptable), and include the associated metric (p95, TPM, timeout). (B) Triggers WITHOUT metrics (timers, webhooks, deployments, security events, internal events): sufficient to name the trigger and what part of the system handles it — no exact endpoint, retry policy, or cooldown required. The validator must NOT require in any case: autoscaling policies (threshold, cooldown, min/max replicas), HTTP response codes, retry or backoff policies, detailed failover mechanisms, or rollback behavior. These are solution details, not diagnosis details.",
...
def build_repair_prompt(index: int, lang: str, reason: str = "") -> str:
    ...
    if _lang == "es":
        templates = {
            ...
            3: "Reescribe tu respuesta describiendo el evento específico que dispara el comportamiento. Si tiene métricas (latencia, TPM, usuarios concurrentes, timeouts), indica qué parte del sistema o funcionalidad lo recibe (por ejemplo, 'la búsqueda', 'el flujo de reserva', 'el procesamiento de pagos') y si la interacción es síncrona o asíncrona. Si no tiene métricas (timer, webhook, despliegue, actor malicioso), basta con nombrar el evento de forma concreta.",
        ...
        templates = {
            ...
            3: "Rewrite your answer describing the specific event that triggers the behavior. If it has associated metrics (latency, TPM, concurrent users, timeouts), indicate what part of the system or functionality receives it (e.g. 'the search functionality', 'the reservation flow', 'payment processing') and whether the interaction is sync or async. If it has no metrics (timer, webhook, deployment, malicious actor), naming the event specifically is sufficient.",
