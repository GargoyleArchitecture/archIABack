...
"""
    ...
    active["arch_stage"] = "TACTICS"
    active["quality_attribute"] = qa
    active["current_asr"] = asr_text
    active["tactics"] = state.get("tactics_list", [])
    log_entries = list(active.get("decision_log") or [])
    for name in state.get("tactics_list", [])[:3]:
        log_entries.append({"type": "tactic", "value": _clip_text(name, 40), "source_turn": len(state.get("messages", []))})
    active["decision_log"] = log_entries[-20:]
