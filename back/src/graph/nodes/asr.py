...
"""
    ...
    active["arch_stage"] = "ASR"
    active["quality_attribute"] = concern
    active["current_asr"] = content
    log = list(active.get("decision_log") or [])
    log.append({"type": "asr", "value": _clip_text(content, 120), "source_turn": len(state.get("messages", []))})
