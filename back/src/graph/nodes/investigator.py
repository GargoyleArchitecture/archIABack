...
from src.graph.utils import _push_turn, _last_k_messages, _clip_text, _merge_context_text, _resolve_response_language
...
def researcher_node(state: GraphState) -> GraphState:
    ...
    if msgs_out:
        ...
        # Merge no-destructivo para evitar perder contexto en conversaciones largas
        prev_ctx = state.get("add_context", "")
        merged = _merge_context_text(prev_ctx, _clip_text(str(last_text).strip(), 1200), max_chars=4000)
        state["add_context"] = merged

        facts = dict(state.get("context_facts") or {})
        if merged and not facts.get("domain"):
            first_line = merged.splitlines()[0].strip()
            facts["domain"] = first_line[:180]
