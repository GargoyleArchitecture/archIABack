...
# Centralizar constantes para mejorar la cohesión y evitar duplicidad
DEFAULT_VIDEO_EXTENSIONS = {'.mp4', '.avi', '.mov', '.mkv', '.wmv', '.flv'}
...
class ArchIAFileHandler(FileSystemEventHandler):
    ...
    def __init__(
        ...
        self.video_extensions = DEFAULT_VIDEO_EXTENSIONS

        # Track processed files to avoid reprocessing
        self.processed_files: set[str] = set()
        # Usar ruta absoluta basada en la ubicación del script o raíz
        self.manifest_path = Path(__file__).parent.resolve() / ".processed_files.json"
        self._load_manifest()
...
class ArchIAWatcher:
    ...
    def __init__(
        ...
        # Obtenemos las rutas de configuración (ej: "back/docs")
        raw_docs = EVAL_CONFIG["docs_dir"]
        raw_videos = EVRAG_CONFIG["videos_dir"]

        # Alta Cohesión: Aseguramos que las rutas sean relativas a la raíz del proyecto
        # Si el usuario ejecuta desde la raíz, "back/docs" funciona.
        # Si por alguna razón la ruta no existe, intentamos normalizarla.
        self.docs_dir = docs_dir or Path(raw_docs)
        self.videos_dir = videos_dir or Path(raw_videos)
    ...
    def scan_existing_files(self) -> dict[str, list[str]]:
        ...
        print(f"   Documentos: {self.docs_dir.resolve()}")
        print(f"   Videos:     {self.videos_dir.resolve()}")
        ...
        if self.videos_dir.exists():
            for video_ext in DEFAULT_VIDEO_EXTENSIONS:
